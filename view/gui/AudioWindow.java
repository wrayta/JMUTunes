...
public class AudioWindow extends JFrame {
	...
	public AudioWindow() {
		...
		setTitle("JMUTunes Audio Player CS239 (Spring 2013)");
		...
		listener = new TunesListener(this);

		// Build the panels.
		buildMainPanelNormal(list);
		// buildGridPanel();
	...
	public AudioWindow(model.AudioList newLs) {
		list = newLs;
	...
	private void buildMainPanelNormal(model.AudioList list) {
	...
	private void buildMainPanelSearch(model.AudioList list) {
		// create the defaultListModel
		model = getList().getModel();
	...
	private void buildSearchButton() {
		...
		editAndDeletePanel.add(searchButton);
	}
